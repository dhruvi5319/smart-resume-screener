...
//           <div className="max-w-2xl mx-auto">
//           <JobDescriptionForm onSubmit={(job) => handleJobDescriptionSubmit(job)} />
//           </div>
...
import { uploadResume, fetchCandidates, saveJobDescription, fetchJobDescriptions } from "@/services/api";
...
const Dashboard = () => {
  ...
  const handleResumeUpload = async (file: File, jobDescriptionId: number) => {
    try {
      const candidate = await uploadResume(file, jobDescriptionId);
  ...
  // const handleJobDescriptionSubmit = async (jobDesc: Omit<JobDescriptionType, "id" | "createdAt">) => {
  //   try {
  ...
  const handleJobDescriptionSubmit = async (jobDesc: Omit<JobDescriptionType, "id" | "createdAt">) => {
    try {
      const response = await saveJobDescription(jobDesc); // Send without id
      const savedJob = response.data; // Receive saved job with backend-generated id
  
      const newJob = {
        ...savedJob, // Includes backend-generated id and createdAt
      };
  
      setJobDescriptions((prev) => [...prev, newJob]);
      console.log("✅ Saved job description:", newJob);
    } catch (error) {
      toast({
        title: "Failed to save job description",
        description: "Please try again later.",
    ...
    <div className="container mx-auto py-8 px-4 bg-gradient-to-b from-blue-50 to-white min-h-screen">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {candidates.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {candidates.map((candidate) => (
                <CandidateCard key={candidate.id} candidate={candidate} onViewDetails={setSelectedCandidate} />
              ))}
            </div>
          ) : (
            <p className="text-center">No candidates yet. Upload a resume to begin.</p>
          )}
        </TabsContent>
        ...
        <TabsContent value="candidate-result">
          {newlyAddedCandidate && (
            <CandidateCard candidate={newlyAddedCandidate} onViewDetails={setSelectedCandidate} />
          )}
        </TabsContent>
      </Tabs>
          ...
          {selectedCandidate && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{selectedCandidate.jobTitle}</DialogDescription>
              </DialogHeader>
